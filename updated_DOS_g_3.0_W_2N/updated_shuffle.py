...
for attempt in range(1, max_attempts + 1):
    ...
    try:
        ...
        # Add APD + sigmoid parameters to CSV
        ...
        # Output filename tag
        # -----------------------------
        tag = f"sharp{sharpness:.2e}_width{width}_amp{amplitude}"
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'apd90_hist_{tag}_attempt{attempt}.pdf')
        plt.close()

        plot_ap_traces(
            d, time, list(range(N)),
            title=f'AP Traces\nsharpness={sharpness:.4f}, width={width}, amplitude={amplitude}',
            save_path=f'ap_traces_{tag}_attempt{attempt}.pdf'
        )

        plot_corona_ring(
            d, time, N,
            title=f'Corona Ring\nsharpness={sharpness:.4f}, width={width}, amplitude={amplitude}',
            save_path=f'ap_circular_heatmap_{tag}_attempt{attempt}.png'
        )

        df_combined.to_csv(f'params_{tag}_attempt{attempt}.csv', index=False)

        with open(f'metadata_{tag}_attempt{attempt}.txt', 'w') as f:
